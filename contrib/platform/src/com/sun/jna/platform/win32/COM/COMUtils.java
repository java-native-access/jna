...
public abstract class COMUtils {
	...
	public static boolean SUCCEEDED(HRESULT err) throws AutomationException {
		return SUCCEEDED(err.intValue());
	}

	public static boolean SUCCEEDED(int err) {
		switch (err) {
		case S_OK:
			return true;
		case E_NOTIMPL:
		...
		case E_UNEXPECTED:
			throw new AutomationException("Error " + Integer.toHexString(err));
		case CO_E_NOTINITIALIZED:
			throw new AutomationException("CoInitialized wasn't called!");
		case E_NOINTERFACE:
			throw new AutomationException("Interface does not inherit from class!");
		case E_POINTER:
		...
		case DISP_E_MEMBERNOTFOUND:
			throw new AutomationException("The requested member does not exist!");
		...
		default:
			throw new AutomationException("Unexpected COM error code : " + err);
		}
	}
}
