...
public interface User32 extends StdCallLibrary, WinUser, WinNT {
	 ...
	 *            If this parameter is HWND_BROADCAST ((HWND)0xffff), the
	 *            message is sent to all top-level windows in the system,
	 *            including disabled or invisible unowned windows.<br>
	 ...
	 *            <ul>
	 *            <li>SMTO_ABORTIFHUNG (0x0002): The function returns without
	 *            waiting for the time-out period to elapse if the receiving
	 *            thread appears to not respond or "hangs."</li>
	 *            <li>SMTO_BLOCK (0x0001): Prevents the calling thread from
	 *            processing any other requests until the function returns.</li>
	 *            <li>SMTO_NORMAL (0x0000): The calling thread is not prevented
	 *            from processing other requests while waiting for the function
	 *            to return.</li>
	 *            <li>SMTO_NOTIMEOUTIFNOTHUNG (0x0008): The function does not
	 *            enforce the time-out period as long as the receiving thread is
	 *            processing messages.</li>
	 *            <li>SMTO_ERRORONEXIT (0x0020): The function should return 0 if
	 *            the receiving window is destroyed or its owning thread dies
	 *            while the message is being processed.</li>
	 *            </ul>
	 * @param uTimeout
	 *            The duration of the time-out period, in milliseconds. <br>
	 *            If the message is a broadcast message, each window can use the
	 *            full time-out period.<br>
	 *            For example, if you specify a five second time-out period and
	 *            there are three top-level windows that fail to process the
	 *            message, you could have up to a 15 second delay.
	 * @param lpdwResult
	 *            Type: PDWORD_PTR <br>
	 *            The result of the message processing. The value of this
	 *            parameter depends on the message that is specified.
	 ...
	 *         SendMessageTimeout does not provide information about individual
	 *         windows timing out if HWND_BROADCAST is used.<br>
	 *         If the function fails or times out, the return value is 0.<br>
	 *         To get extended error information, call GetLastError. If
	 *         GetLastError returns ERROR_TIMEOUT, then the function timed out.
	 */
	LRESULT SendMessageTimeout(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam, UINT fuFlags, UINT uTimeout,
	 ...
	 */
	HANDLE SetWinEventHook(UINT eventMin, UINT eventMax, HMODULE hmodWinEventProc, WinUser.WinEventProc winEventProc,
			DWORD processID, DWORD threadID, UINT flags);
	 ...
	 *            SetWinEventHook.
	 */
	void UnhookWinEvent(HANDLE hook);
}
